...
roi(1).name = {'rPT'}; roi(1).predictors = []; % initialize the design matrix for roi voxels
roi(2).name = {'lMT'}; roi(2).predictors = []; % initialize the design matrix for roi voxels
roi(3).name = {'rMT'}; roi(3).predictors = []; % initialize the design matrix for roi voxels
...
factor(1).col = 2; factor(1).classlabels = [];
factor(1).labels = {'Seq', 'Onset', 'Random'};
factor(1).chance = 1/3;
factor(2).col = 3; factor(2).classlabels = [];
factor(2).labels =  {'left', 'right'};
factor(2).chance = 1/2;
% create an empty factor that will be the combination of previous factors
factor(3).col = NaN;
factor(3).labels = NaN; 
factor(3).classlabels = [];
factor(3).chance = 1/6;
...
for s = 1:length(paths.session) % for each session
    ...
    for i = 1:length(datafilelist) % for each vmp/glm file
        ...
        % load exp protocols
        emat = mvpa.load_exp(datafilelist(i).name, paths);
        ...
        data_roi = mvpa.subset_data_rois(data_xff, roi_xff, dataformat); % just save the data for ROIs, structure
